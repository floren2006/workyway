                ...
                <div class="col-md-8">
                    <form method="GET" action="<?php echo base_url('admin/manajemen_pengguna'); ?>">
                        <div class="input-group">
                            <input type="text" class="form-control" name="search" placeholder="Cari pengguna berdasarkan nama atau email..." value="<?php echo isset($search) ? htmlspecialchars($search) : ''; ?>">
                            <button class="btn btn-primary" type="submit">
                                <i class="fas fa-search"></i> Cari
                            </button>
                        </div>
                        <input type="hidden" name="filter" id="filter-input" value="<?php echo isset($filter) ? htmlspecialchars($filter) : ''; ?>">
                    </form>
                    
                    <div class="filter-buttons">
                        <button type="button" class="filter-btn <?php echo (!isset($filter) || $filter == '' || $filter == 'all') ? 'active' : ''; ?>" data-filter="all">Semua</button>
                        <button type="button" class="filter-btn <?php echo (isset($filter) && $filter == 'siswa') ? 'active' : ''; ?>" data-filter="siswa">Siswa</button>
                        <button type="button" class="filter-btn <?php echo (isset($filter) && $filter == 'lpk') ? 'active' : ''; ?>" data-filter="lpk">LPK</button>
                        <button type="button" class="filter-btn <?php echo (isset($filter) && $filter == 'guru') ? 'active' : ''; ?>" data-filter="guru">Guru</button>
                        <button type="button" class="filter-btn <?php echo (isset($filter) && $filter == 'aktif') ? 'active' : ''; ?>" data-filter="aktif">Status Aktif</button>
                        <button type="button" class="filter-btn <?php echo (isset($filter) && $filter == 'pending') ? 'active' : ''; ?>" data-filter="pending">Status Pending</button>
                        <button type="button" class="filter-btn <?php echo (isset($filter) && $filter == 'nonaktif') ? 'active' : ''; ?>" data-filter="nonaktif">Status Nonaktif</button>
                    </div>
                </div>
            </div>
        </div>
